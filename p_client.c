...
void player_die(edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, vec3_t point)
{
	...
	if (!self->deadflag)
	{
		...
		if (meansOfDeath != MOD_CHANGETEAM && meansOfDeath != MOD_CHANGETEAM_WOUNDED) /* MetalGod Incorrect operator:  mutual exclusion over || is always a non-zero constant changed to && */
		{
...
void ClientDisconnect(edict_t* ent)
{
	...
	/* MetalGod moved these down here to avoid sending to free/zombie client */
	stuffcmd(ent, "cl_forwardspeed 200;cl_sidespeed 200;cl_upspeed 200;");
	...
	*/
	...
	if (!chile->value && ent->client && ent->client->pers.weapon)
	{
		...
		if (time < 30)
		{
			...
			{
				if ((time < 20) && time != 0)
					ent->client->kick_angles[0] = ((2 * cosf((time / 4) - 4)) * ((20 - time) / 20));
			}
			else if (ent->client->pers.weapon->position == LOC_RIFLE)
			{
				if ((time < 2) && time != 0)
					ent->client->kick_angles[0] = ((2 * cosf((time)-4)) * ((20 - time) / 20));
			}
			...
			{
				if ((time < 2) && time != 0)
					ent->client->kick_angles[0] = ((3 * cosf((time)-4)) * ((20 - time) / 20));
			}
		}
	}
	...
	if (ent->client->resp.scopewobble < 40)
	{
		...
		{
			// X Axis
			ent->client->kick_angles[0] = sinf(ent->client->resp.scopewobble * 10) * (1 - (float)(ent->client->resp.scopewobble) / 40) * 2.5;
			// Y Axis
			ent->client->kick_angles[1] = sinf((ent->client->resp.scopewobble + 45) * 6) * (1 - (float)(ent->client->resp.scopewobble) / 40) * 2.5;
