...
void Cmd_RemoveDoors(edict_t* end)//for help with adding bot support
{
	...
	if (sv_cheats->value != 0)
	{
		...
		for (e = 1; e < globals.num_edicts; e++, check++)
		{
			...
			if (!check->classnameb)
				continue;
			if (check->classnameb != FUNC_DOOR_ROTATING) /* MetalGod moved here for clarity */
				continue;
...
qboolean Cmd_Scope_f(edict_t* ent)
{
	...
	if (!ent->client->pers.weapon || ent->client->pers.weapon->classnameb == WEAPON_SANDBAGS ||
...
void Cmd_Create_Team(edict_t *ent)
{
	...
	if(!found) safe_cprintf(ent,PRINT_HIGH,"Unable to create team.\n");
	else safe_bprintf(PRINT_CHAT,"Team %s created.\n",teamname);
}
...
qboolean Cmd_Reload(edict_t* ent)
{
	...
	int		ammo_index, * ammo_amount = NULL; /* MetalGod Initialized */
...
void Cmd_Shout_f(edict_t* ent)
{
	...
	if (!strcmp(shout, "thanks1") ||
		...
		!strcmp(shout, "eye1") ||
		/* !strcmp(shout, "thanks1") || MetalGod used above */
		!strcmp(shout, "smoke1") ||
		!strcmp(shout, "help1"))
	{
		newshout = false;
